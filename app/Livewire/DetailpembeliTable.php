...
final class DetailpembeliTable extends PowerGridComponent
{
    ...
    public function actions(\App\Models\Detailpembeli $row): array
    {
        return [
            Button::add('edit')
                ...
                ->openModal('detailpembeli-form', ['rowId' => $row->id]),
            Button::add('delete')
                ->slot('<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.detailpembeli', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/detailpembeli.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/detailpembeli.pdf');
    }
