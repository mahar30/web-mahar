...
final class UkuranTable extends PowerGridComponent
{
    use WithExport;
    ...
    {
        return Ukuran::query()
            ->leftJoin('barang', 'ukuran.barang_id', '=', 'barang.id')
            ->select('ukuran.*', 'barang.nama_barang as nama_barang');
    }
    ...
    public function actions(\App\Models\Ukuran $row): array
    {
        return [
            Button::add('edit')
                ...
                ->openModal('ukuran-form', ['rowId' => $row->id]),
            Button::add('delete')
                ->slot('<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.ukuran', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/ukuran.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/ukuran.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $ukuran = Ukuran::findOrFail($rowId);
